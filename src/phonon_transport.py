...
class PhononTransport:
	...
	def __init__(self, data_path, coord_path, n_l, n_r, gamma, in_plane, eigenchannel, every_nth,
				 channel_max):
		...
		self.w = np.linspace(0.0, w_D * 1.1, N)
		self.i = np.linspace(0, N, N, False, dtype=int)
		g0 = self.calculate_g0(w, w_D)
		self.Sigma = self.calculate_Sigma(w, g0, gamma, M_L, M_C)
	...
	def calculate_T_i(self, i):
		...
		if (eigenchannel == True):
			...
			T, T_channel = self.calc_eigenchannel(trans_prob_matrix, data_path, channel_max, coord, write_out, energy)
			return T, T_channel
		else:
...
if __name__ == '__main__':
	...
	PT = PhononTransport(data_path, filename_coord, n_l, n_r, gamma, in_plane, eigenchannel, every_nth,
	channel_max)
